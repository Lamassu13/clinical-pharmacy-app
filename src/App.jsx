...
function App() {
  ...
  const reconcileChartConflict = useCallback(async (date) => {
    ...
    const freshPatients = Array(PATIENT_ROWS).fill('')
    const freshQuantities = Array.from({ length: PATIENT_ROWS }, () => Array(CHART_COLUMNS).fill(''))
    const freshColumns = Array(CHART_COLUMNS).fill('')
    if (result.chart) {
      result.chart.patients.forEach((patient) => { freshPatients[patient.row_number - 1] = patient.patient_name })
      result.chart.quantities.forEach((quantity) => { freshQuantities[quantity.row_number - 1][quantity.column_number - 1] = String(quantity.quantity) })
      result.chart.columns.forEach((column) => { freshColumns[column.column_number - 1] = column.medicine_name || '' })
    }
    const mergedPatients = freshPatients.map((value, index) => (patientNames[index] !== base.patientNames[index] ? patientNames[index] : value))
    const mergedColumns = freshColumns.map((value, index) => (columnMedicines[index] !== base.columnMedicines[index] ? columnMedicines[index] : value))
    const mergedQuantities = freshQuantities.map((row, rowIndex) => row.map((value, columnIndex) => (quantities[rowIndex][columnIndex] !== base.quantities[rowIndex]?.[columnIndex] ? quantities[rowIndex][columnIndex] : value)))
    setPatientNames(mergedPatients)
    setColumnMedicines(mergedColumns)
    setQuantities(mergedQuantities)
    chartVersionRef.current = result.chart ? result.chart.version : 0
    lastSyncedChartRef.current = { patientNames: mergedPatients, columnMedicines: mergedColumns, quantities: mergedQuantities }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (cancelled) return
        if (!result.chart) {
          const blankPatients = Array.from({ length: PATIENT_ROWS }, () => '')
          const blankQuantities = Array.from({ length: PATIENT_ROWS }, () => Array(CHART_COLUMNS).fill(''))
          const blankColumns = Array(CHART_COLUMNS).fill('')
          setPatientNames(blankPatients); setQuantities(blankQuantities); setColumnMedicines(blankColumns)
          chartVersionRef.current = 0
          lastSyncedChartRef.current = { patientNames: blankPatients, columnMedicines: blankColumns, quantities: blankQuantities }
        } else {
          ...
          chartVersionRef.current = result.chart.version
          lastSyncedChartRef.current = { patientNames: nextPatients, columnMedicines: nextColumns, quantities: nextQuantities }
        }
