...
function App() {
  ...
  // True while the software keyboard is up, so the chart can give the grid the space the
  // toolbars were using.
  ...
  // The chart screen is sized to the *visual* viewport rather than to 100svh, because on
  // iOS opening the keyboard shrinks only the visual one. With the screen exactly as tall
  // as what can be seen, the document has nothing to scroll, so the keyboard cannot push
  // the medicine header off the top: the only thing that scrolls is the grid, and the
  // header is not inside it. Compensating for the offset was tried instead and was worse —
  // it dragged the info bar down over the cells being typed into.
  useEffect(() => {
    ...
    const apply = () => {
      ...
      // Writing a custom property on the root invalidates style for the whole document,
      // and this grid holds 2000+ inputs. The height only moves on rotate and on the
      // keyboard opening or closing, so guard on the value.
      if (height === lastHeight) return
      lastHeight = height
      document.documentElement.style.setProperty('--app-vh', `${height}px`)
      // Well past any browser-chrome change, comfortably under any keyboard.
