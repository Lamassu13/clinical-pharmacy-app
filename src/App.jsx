...
function MedicineRow({ item, onSave, onRemove }) {
  ...
  return <tr>
    ...
    <td className="requests-actions"><button className="primary-button compact" disabled={!dirty} onClick={() => onSave(item.id, name.trim(), arabicName.trim())}>حفظ</button><button className="danger-button compact" onClick={() => onRemove(item.id, item.name)}>حذف</button></td>
  </tr>
}
