...
function App() {
  ...
  const approveRegistration = useCallback(async (id) => {
    const floor = Number(pendingFloor[id])
    if (!floors.some((item) => item.number === floor)) { setRegistrationsError('اختر الطابق المسموح قبل قبول الطلب'); return }
    setRegistrationsError('')
    try {
      const response = await fetch(`${apiUrl}/registrations/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ status: 'active', floor }) })
      const result = await response.json()
  ...
  const assignFloorToUser = useCallback(async (id, floorValue) => {
    const floor = Number(floorValue)
    if (!floors.some((item) => item.number === floor)) return
    setRegistrationsError('')
    try {
      const response = await fetch(`${apiUrl}/access/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ floor }) })
      const result = await response.json()
      if (!response.ok) throw new Error(result.message || 'تعذر تعيين الطابق')
      setAllUsers((current) => current.map((user) => user.id === id ? { ...user, floors: [floor] } : user))
    } catch (error) { setRegistrationsError(error.message || 'تعذر الاتصال بالخادم') }
  }, [])
  ...
  useEffect(() => { if (adminView) { loadRegistrations(); loadUsers() } }, [adminView, loadRegistrations, loadUsers])
  ...
  if (adminView && currentUser?.role === 'admin') return <main className="app-shell"><header className="topbar"><div className="topbar-brand"><img className="hospital-logo header-logo" src={hospitalLogo} alt="شعار مستشفى بغداد التعليمي" /><div><strong>الصيدلة السريرية</strong><small>مستشفى بغداد التعليمي</small></div></div><div className="user-menu"><button onClick={() => setAdminView(false)} className="text-button">→ عودة</button></div></header><section className="dashboard"><div className="section-heading"><div><p className="eyebrow">إدارة الحسابات</p><h1>طلبات الانضمام</h1><p>عند القبول اختر الطابق المسموح للمستخدم.</p></div><button className="secondary-button" onClick={() => { loadRegistrations(); loadUsers() }}>تحديث</button></div>{registrationsError && <p className="form-error">{registrationsError}</p>}{registrations.length === 0 ? <p className="login-intro">لا توجد طلبات قيد الانتظار.</p> : <div className="table-frame"><table className="requests-table"><thead><tr><th>الاسم الكامل</th><th>اسم المستخدم</th><th>الهاتف</th><th>البريد الإلكتروني</th><th>رقم البصمة</th><th>الطابق</th><th>إجراء</th></tr></thead><tbody>{registrations.map((item) => <tr key={item.id}><td>{item.full_name}</td><td>{item.username}</td><td>{item.phone}</td><td>{item.email}</td><td>{item.fingerprint_number}</td><td><select value={pendingFloor[item.id] || ''} onChange={(event) => setPendingFloor((current) => ({ ...current, [item.id]: event.target.value }))}><option value="">اختر</option>{floors.map((floorOption) => <option key={floorOption.number} value={floorOption.number}>{floorOption.number}</option>)}</select></td><td className="requests-actions"><button className="primary-button compact" onClick={() => approveRegistration(item.id)}>قبول</button><button className="secondary-button compact" onClick={() => rejectRegistration(item.id)}>رفض</button></td></tr>)}</tbody></table></div>}<div className="section-heading admin-users-heading"><div><p className="eyebrow">المستخدمون</p><h1>جميع المستخدمين</h1><p>الحسابات المسجلة في النظام.</p></div></div>{allUsers.length === 0 ? <p className="login-intro">لا يوجد مستخدمون.</p> : <div className="table-frame"><table className="requests-table"><thead><tr><th>الاسم الكامل</th><th>اسم المستخدم</th><th>البريد الإلكتروني</th><th>الهاتف</th><th>الدور</th><th>الحالة</th><th>الطوابق</th><th>إجراء</th></tr></thead><tbody>{allUsers.map((user) => <tr key={user.id}><td>{user.full_name}</td><td>{user.username}</td><td>{user.email}</td><td>{user.phone}</td><td>{user.role === 'admin' ? 'مدير' : 'مستخدم'}</td><td>{({ pending: 'قيد الانتظار', active: 'مفعل', rejected: 'مرفوض', suspended: 'موقوف' })[user.account_status] || user.account_status}</td><td><select value={(user.floors || [])[0] || ''} onChange={(event) => assignFloorToUser(user.id, event.target.value)}><option value="">بدون</option>{floors.map((floorOption) => <option key={floorOption.number} value={floorOption.number}>{floorOption.number}</option>)}</select></td><td className="requests-actions">{user.id !== currentUser.id && <button className="secondary-button compact" onClick={() => deleteUser(user.id, user.full_name)}>حذف</button>}</td></tr>)}</tbody></table></div>}</section></main>
