...
function App() {
  ...
  const flushPills = useCallback(() => {
    ...
    const entries = Object.entries(pillEntries).map(([key, value]) => {
      ...
      return { patientRowNumber, medicineId, doseTime: value.doseTime || '', usageMethod: value.usageMethod || '', leadNote: value.leadNote || '', note: value.note || '' }
    })
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const seed = {}
        ;(result.pills?.entries || []).forEach((entry) => { seed[`${entry.patientRowNumber}:${entry.medicineId}`] = { doseTime: entry.doseTime || '', usageMethod: entry.usageMethod || '', leadNote: entry.leadNote || '', note: entry.note || '' } })
        setPillEntries(seed)
        setPillsLoadError(false)
