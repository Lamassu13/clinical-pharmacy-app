...
test('GET /api/dashboard dailyPatientsByFloor (manager): per-floor total (summed across its wards) plus one line per special ward, day by day, over a fixed 7-day window regardless of the other widgets\' period', async () => {
  const user = await createUser({ role: 'user', floor: 5 })
  const two = [{ rowNumber: 1, name: 'أ' }, { rowNumber: 2, name: 'ب' }]
  const three = [...two, { rowNumber: 3, name: 'ج' }]
  ...
  const at = async (patientsPeriod, period = 'month') =>
    sorted((await client.get(`/api/dashboard?date=${DATE}&period=${period}&patientsPeriod=${patientsPeriod}`)).body.dailyPatientsByFloor)
