...
export const authenticateUser = async (username, password) => {
  ...
  const result = await query('SELECT u.id, u.full_name, u.username, u.role, u.account_status, u.password_hash, MIN(ufa.floor_number) AS assigned_floor FROM users u LEFT JOIN user_floor_access ufa ON ufa.user_id = u.id WHERE u.username = $1 OR LOWER(u.email) = LOWER($1) GROUP BY u.id', [username])
  ...
  return { id: user.id, fullName: user.full_name, username: user.username, role: user.role, assignedFloor: user.assigned_floor ? Number(user.assigned_floor) : null, assignedWards: wards.rows.map((row) => row.ward_name) }
}
