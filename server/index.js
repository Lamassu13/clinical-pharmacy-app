...
app.delete('/api/users/:id', requireAdmin, async (request, response) => {
  ...
  try {
    ...
    await client.query('UPDATE daily_charts SET completed_by = NULL WHERE completed_by = $1', [id])
    await client.query('UPDATE treatment_forms SET uploaded_by = NULL WHERE uploaded_by = $1', [id])
    await client.query('UPDATE extra_pill_forms SET created_by = NULL WHERE created_by = $1', [id])
